...
- [x] FOLLOW-UP: cross-note [[Note#Heading]] fragments still use slugify and miss goldmark's heading ids — 8 of 28 dangle in the Pattern Zoo note (scripts/06-cross-note-fragment-audit). Needs a per-note heading-id index in the vault layer. <!-- t:ikll -->
...
- [ ] FOLLOW-UP: heading ids for headings containing math embed the math sentinel index (## The $\\sigma$ bound -> id 'the-0-bound'), so adding an earlier formula renumbers the heading's permalink. Pre-existing; links resolve correctly against it. <!-- t:hmdb -->
